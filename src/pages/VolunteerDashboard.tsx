        ...
        <header style={{ marginBottom: 40 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, marginTop: 32 }}>
            {[
              ...
              { label: 'Pending Requests', value: requestedItems.filter(r => r.status === 'pending').length, color: 'var(--success)', iconPath: 'M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z' },
              { label: 'Total Requests', value: requestedItems.length, color: 'var(--accent)', iconPath: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z' },
            ].map(s => (
                      ...
                      <div style={{ padding: '20px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.75rem', color: 'var(--outline)' }}>
                            ...
                            {new Date(donation.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </div>
                        </div>
                ...
                {requestedItems.map(req => (
                  <div key={req.id} className="card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                      ...
                      <span style={{ padding: '4px 12px', borderRadius: 99, fontSize: '0.75rem', fontWeight: 700, background: req.status === 'pending' ? 'var(--primary-fixed)' : 'var(--success-soft)', color: req.status === 'pending' ? 'var(--primary)' : 'var(--success)', flexShrink: 0, marginLeft: 12 }}>
